...
$(document).on('click', '.add_to_cart', function () {
	if ($("#customers").val() == "") {
		...
		if (product_type == "serialize") {
			showSerializTable(product_id, warehouse_id, '', product_id)
			return '';
		}
			...
			if (parseFloat(customerCreditLimit) >= parseFloat(grandTotal)) {
				if (product_quantity > 0) {
					$.ajax({
						url: "phpScripts/whoseSaleAction.php",
						method: "POST",
						data: { product_id: product_id, product_name: product_name, product_discount: product_discount, product_price: product_price, min_price: min_price, max_price: max_price, product_quantity: product_quantity, product_limit: totalQuantity, warehouse_id: warehouse_id, warehouse_name: warehouse_name, action: action, serializeProductsId: serializeProductsId, serializeSaleQuantity: serializeSaleQuantity},
						dataType: 'json',
						success: function (data) {
							if (data.count > 0) {
		...
		if (parseFloat(customerCreditLimit) >= parseFloat(grandTotal)) {
			if (product_quantity > 0) {
				$.ajax({
					url: "phpScripts/whoseSaleAction.php",
					method: "POST",
					data: { product_id: product_id, product_name: product_name, product_discount: product_discount, product_price: product_price, min_price: min_price, max_price: max_price, product_quantity: product_quantity, grandTotal: grandTotal, warehouse_id: warehouse_id, warehouse_name: warehouse_name, action: action, serializeProductsId: serializeProductsId, serializeSaleQuantity: serializeSaleQuantity },
					dataType: 'json',
					success: function (data) {
						if (data.count > 0) {
...
function showSerializTable(id, warehouseId, txt, product_id) {
	...
	$.ajax({
		...
		success: function (result) {
				...
				$("#checkSesssinStatus").val(result.checkSesssinStatus);
				$("#serialNumsModal").modal("show");
				$("#totalStockQuantity").text(result.totalQuantityForSale);
				if ( result.totalQuantityForSale<=0 ) {
					$("#checkSesssinStatus").val("insert");
				}
		},
		beforeSend: function () {
			$('#loading').show();
...
function calculateTotalQuantity(saleQty, product_id, warehouse_id, tblSerializeId) {
	...
	if (saleQty > serializeRemainingQty) {
		checkError = true;
		$("#stockQuantity_" + tblSerializeId).val(1);
		saleQty = 1;
		errorMsg = "available stock quantity [ " + serializeRemainingQty + " ]";
	}
	...
	if (totalStockQuantity <= 0) {
		...
		saleQty = 1;
		$("#stockQuantity_" + tblSerializeId).val(1);
		errorMsg = "Product Quantity cannot be 0(zero)";
	}

	if (checkError) {
		$("#divErrorMsgShow").html("<strong><i class='icon fa fa-trash'></i>Error ! </strong> "+errorMsg+"");
		$("#divErrorMsgShow").show().delay(3000).fadeOut().queue(function (n) {
			$(this).hide(); n();
		});
	}
	...
	let checkSesssinStatus = $("#checkSesssinStatus").val();
	let currentStockId = $("#id"+product_id).val();
	let checkWarehouseId = $("#warehouseId"+currentStockId).val();

	if(typeof currentStockId != "undefined" && warehouse_id == checkWarehouseId){ 
		product_id = currentStockId;
	}

	if (checkSesssinStatus == "insert") {
		selectProducts(product_id, warehouse_id, '');
		$("#checkSesssinStatus").val("update");
	}else{
