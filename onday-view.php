        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-body">
              ...
              <table id="example_onday" class="table table-bordered">
                <thead>
                  <th>ID</th>
                  <th>Date</th>
                  <th>Day Name</th>
                ...
                <tbody>
                  <?php
                    $sql = "SELECT * FROM `calender_tbl` WHERE day_type='Onday'and (date BETWEEN '$firstDayOfYear' AND '$lastDayOfYear')";
                    ...
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          ...
                          <td>
                            <button class='btn btn-success btn-sm ondayedit btn-flat' data-id='".$row['id']."'><i class='fa fa-edit'></i> Edit </button>
                          </td>
                        </tr>
                      ";
                    }
...
<script>
$(function(){
  $('.ondayedit').click(function(e){
    e.preventDefault();
    $('#ondayedit').modal('show');
    var id = $(this).data('id');
    getRow(id);
  });
});
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      $('#ondayid').val(response.id);
      $('#edit_date').val(response.date);
      $('#edit_day_name').val(response.day_name);
	  $('#edit_onday_type').val(response.day_type);
      $('#edit_on_offday_cause').val(response.offday_cause);
...
<script type="text/javascript">
		...
		$(document).ready(function(){
            ...
            var table = $('#example_onday').DataTable();
