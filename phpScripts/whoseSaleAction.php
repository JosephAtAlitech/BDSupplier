...
if (isset($_POST["action"])) {
	if ($_POST["action"] == "add") {
		...
		$data = array('productType' => $productType, 'productId' => $productId, 'warehouseId' => $warehouseId, 'currentStockId' => $currentStockId);
		echo json_encode($data);
	...
	if (!empty($_SESSION["wholeSaleShopping_cart"])) {
		foreach ($_SESSION["wholeSaleShopping_cart"] as $keys => $values) {
			...
			$output .= '<tr  style="background-color: #e1e1e1;font-size: 12px;">
    			...
    			<td><input '.$qtyDisable.' class=" saleQuantity_' . $values["product_id"] . '" type="text" id="productQuantity' . $values["id"] . '" name="productQuantity" value="' . $values["product_quantity"] . '" onkeyup="calculateTotal(' . $values["id"] . ')" onblur="updateSession(' . $values["id"] . ',true)" style="width: 100%;text-align: center;"/>';
			if ($product_type == "serialize") {
