...
if (!empty($_SESSION["shopping_cart"])) {
	foreach ($_SESSION["shopping_cart"] as $keys => $values) {
		...
		if ($product_type == "serialize") {
			$qtyDisable = 'Disabled';
			$showBtn  = '<a href="#" onclick="showSerializTable(' . $values["product_id"] . ', ' . $values["warehouse_id"] . ', ' . $values["product_quantity"] . ', ' . $values["id"] . ')"> <i class="fa fa-edit"> </i> </a>';
		...
		$output .= '
    		<tr style="background-color: #e1e1e1;font-size: 12px;">
    			...
    			<td><input '.$qtyDisable.' class=" saleQuantity_' . $values["product_id"] . '" type="text" id="productQuantity' . $values["id"] . '" name="productQuantity" value="' . $values["product_quantity"] . '" onkeyup="calculateTotal(' . $values["id"] . ')" onblur="updateSession(' . $values["id"] . ')" style="width: 100%;text-align: center;"/>'.$showBtn.'</td>
